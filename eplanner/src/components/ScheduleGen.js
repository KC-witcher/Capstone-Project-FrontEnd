        ...
        {options.length !== 0 ?
          <Box>
            ...
            <Stack className="center" spacing={3}>
              <Box className="box" style={{ width: "75%" }} sx={{ border: 2, borderColor: "darkgrey", borderRadius: 3, padding: 1 }} onClick={optionOneClickOpen}>
                {options[0].schedule.split('\n').map(str => <Stack>{str}</Stack>)}
              </Box>
              <Box className="box" style={{ width: "75%" }} sx={{ border: 2, borderColor: "darkgrey", borderRadius: 3, padding: 1 }} onClick={optionTwoClickOpen}>
                {options[1].schedule.split('\n').map(str => <Stack>{str}</Stack>)}
              </Box>
              {options[2].schedule !== "" ?
                <Box className="box" style={{ width: "75%" }} sx={{ border: 2, borderColor: "darkgrey", borderRadius: 3, padding: 1 }} onClick={optionThreeClickOpen}>
                  {options[2].schedule.split('\n').map(str => <Stack>{str}</Stack>)}
                </Box>
                : <></>}
              {options[3].schedule !== "" ?
                <Box className="box" style={{ width: "75%" }} sx={{ border: 2, borderColor: "darkgrey", borderRadius: 3, padding: 1 }} onClick={optionFourClickOpen}>
              ...
              <Dialog open={open}>
                ...
                <DialogContent>
                  <DialogContentText>
                    ...
                    {options[n].schedule.split('\n').map(str => <Stack sx={{ fontWeight: "bold" }}>{str}</Stack>)}
                  </DialogContentText>
                </DialogContent>
                <DialogActions>
